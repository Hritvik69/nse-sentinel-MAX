...
_NO_DATA_TTL_SEC = 2 * 60 * 60
_NO_DATA_TICKERS: dict[str, float] = {}
...
def _apply_time_travel_cutoff(df: pd.DataFrame | None) -> pd.DataFrame | None:
    """Trim a frame to the active TT cutoff when time-travel is enabled."""
    try:
        if df is None or df.empty:
            return None
        from time_travel_engine import apply_time_travel_cutoff

        trimmed = apply_time_travel_cutoff(df)
...
def _prepare_loaded_frame(df: pd.DataFrame | None) -> pd.DataFrame | None:
    """
    Normalize Yahoo/CSV output first, then apply TT trimming.
    Live downloads still require at least 30 rows pre-trim, but TT may shorten
    the usable frame after the cutoff is applied.
    """
    normalized = _normalize_ohlcv_frame(df)
    if normalized is None:
        return None
    trimmed = _apply_time_travel_cutoff(normalized)
    if trimmed is None or trimmed.empty:
        return None
    return trimmed


def _prune_no_data_cache(now_ts: float | None = None) -> None:
    now_ts = time.time() if now_ts is None else float(now_ts)
    cutoff = now_ts - _NO_DATA_TTL_SEC
    expired = [ticker_ns for ticker_ns, ts in _NO_DATA_TICKERS.items() if ts < cutoff]
    for ticker_ns in expired:
        _NO_DATA_TICKERS.pop(ticker_ns, None)


def _has_recent_no_data(ticker_ns: str) -> bool:
    with _NO_DATA_LOCK:
        _prune_no_data_cache()
        ts = _NO_DATA_TICKERS.get(ticker_ns)
        return ts is not None


def _mark_no_data(ticker_ns: str) -> None:
    with _NO_DATA_LOCK:
        _NO_DATA_TICKERS[ticker_ns] = time.time()
...
def preload_all(
    ...
    with _NO_DATA_LOCK:
        _prune_no_data_cache()
        known_no_data = set(_NO_DATA_TICKERS)
